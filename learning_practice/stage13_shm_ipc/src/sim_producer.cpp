...
static void render_frame(cv::Mat& img, const uint64_t seq, const double t) {
    ...
    for (int r = 0; r < img.rows; ++r) {
        img.row(r) = cv::Scalar((r + static_cast<int>(t * 60)) & 0xFF, // B
    ...
    const int x = static_cast<int>(
        (std::sin(t * 0.8) * 0.5 + 0.5) * (img.cols - 200)); // t → [0, cols-200]
...
int main() {
    ...
    fmt::print("共享内存: {} + {}（各 {:.1} MB）\n",
               ipc::shm_path(ipc::SHM_NAME_META).string(),
               ipc::shm_path(ipc::SHM_NAME_IMAGE_POOL).string(),
               ipc::IMAGE_POOL_SIZE / 1024.0 / 1024.0);
    std::fflush(stdout);
    ...
    fmt::print("[ OK ] 共享内存已建立（magic={:#x} version={} 图像 {}x{}）\n",
               ipc::SHM_MAGIC, ipc::SHM_VERSION, ipc::IMAGE_WIDTH, ipc::IMAGE_HEIGHT);
    std::fflush(stdout);
    ...
    while (!g_stop) {
        ...
        const ipc::ShmClient::Pose gimbal_pose{
            ...
            .x = 0.0, .y = 0.0, .z = 0.0,
            .qw = std::cos(yaw / 2), .qx = 0.0, .qy = 0.0, .qz = std::sin(yaw / 2),
            .frame_seq = seq,
            .timestamp_ns = now_ns(),
        };
        ...
        if (now >= next_stat) {
            next_stat += 1s;
            fmt::print("[SIM ] 已发布 {} 帧 | 收到指令 {} 条 | 最新: yaw={:+7.2f}° "
                       "pitch={:+7.2f}° fire={}\n",
                       seq + 1, cmds_received, last_cmd_yaw, last_cmd_pitch,
                       last_cmd_fire ? "YES" : "no");
            std::fflush(stdout);
        }
