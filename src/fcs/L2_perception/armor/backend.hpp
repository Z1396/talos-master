...
// 只告诉编译器：有个叫cv::Mat的类，本文件只用到它的引用，不需要完整定义
namespace cv {
class Mat;
}
...
// ============================================================================
// 前向声明：只告诉编译器类名，不展开类实现，解决头文件互相include循环依赖
class TraditionalBackend;  // 传统图像处理后端（轮廓/灯条匹配，无AI模型）
class OrtBackend;          // ONNX Runtime CPU/GPU推理后端
// 编译开启TensorRT硬件加速宏才声明这个类；不开启则直接裁剪掉这个类型
#if TALOS_HAS_TENSORRT
class TrtBackend;          // 英伟达TensorRT GPU推理后端
#endif
// 编译开启AXERA国产NPU宏才声明
#if TALOS_HAS_AXERA
class AxeraBackend;        // 国产AXERA NPU推理后端（瑞芯微/爱芯模组常用）
#endif

// 配置结构体前向声明，定义在config.hpp中
struct ArmorDetectorConfig;       // 顶层装甲检测器总配置（里面包含选择哪一种后端、模型路径、阈值等）
#if TALOS_HAS_TENSORRT
struct TrtConfig;                 // TensorRT后端专属参数（引擎路径、FP16/INT8、显存策略）
#endif
#if TALOS_HAS_AXERA
struct AxeraConfig;               // Axera NPU后端专属参数（模型bin、NPU核数）
#endif
struct ArmorTraditionalConfig;    // 传统视觉算法参数（灯条面积阈值、长宽比）
struct ArmorOrtConfig;             // ONNX Runtime推理参数（线程数、GPU设备ID）
 ...
 *
 * 本质：std::variant 类型安全联合体，同一时刻只会持有其中**一种**后端unique_ptr
 * 编译期根据宏自动增减类型列表，不用改业务代码就能裁剪/开启硬件后端
 */
 ...
 * 【核心设计】不用virtual虚函数，用variant+visit做静态分发，无虚表开销，适合机器人实时感知链路
 */
class DetectorBackend {
public:
    /**
     * @brief 检测结果别名：承载装甲目标数组，失败返回错误字符串
     * 成功 → std::vector<ArmorDetection> 装甲列表；失败 → std::string错误信息
     */
    using DetectionResult = std::expected<std::vector<ArmorDetection>, std::string>;

    /**
     * @brief 默认构造，variant初始为空无后端
     * 构造后必须调用set_backend或者带参构造装入后端才能使用
     */
    DetectorBackend();

    /**
     * @brief 析构，variant自动释放内部后端智能指针资源
     * unique_ptr自动释放推理引擎、模型显存资源，不用手动清理
     */
    ~DetectorBackend();

    /**
     * @brief 移动构造，允许检测器实例所有权转移，无内存拷贝
     * noexcept：保证不会抛出异常，适合调度器、工厂函数转移实例
     */
     ...
     * std::holds_alternative：variant标准API，判断里面当前存的是哪一种类型
     */
     ...
     * @return 底层后端左值引用
     * @warning 调用前必须先用holds<>()判断类型，否则std::get直接抛异常
     * 适用场景：后端专属参数调参、NPU状态读取（上层尽量少用，优先统一detect接口）
     */
    template <typename Backend>
...
private:
    DetectorBackendVariant backend_; ///< 存储当前激活的推理后端变体（核心成员）
};
